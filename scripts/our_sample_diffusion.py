...
# from models.our_model_45 import ScorePosNet3D, log_sample_categorical
# 动态导入模型
import importlib
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='../configs/our_sampling.yml')
    parser.add_argument('-i', '--data_id', type=str, default='0-99')
    parser.add_argument('--device', type=str, default='cuda:0')
    parser.add_argument('--batch_size', type=int, default=100)
    parser.add_argument('--tag', type=str, default='')
    parser.add_argument('--sample_logdir', type=str, default='../logs/')
    parser.add_argument('--data_path', type=str, default='../data/')
    parser.add_argument('--model_idx', type=str, default='5351')
    parser.add_argument('--diffusion_path', type=str, default='../result/0.7-all-64/our_training_20241031_194911/checkpoints/best.pt')
    ...
    ScorePosNet3D = import_model(model_path, model_type="ScorePosNet3D")
    log_sample_categorical = import_model(model_path, model_type="log_sample_categorical")
    ...
    # copy model files
    path = os.path.abspath(sys.modules[ScorePosNet3D.__module__].__file__)
    ...
    # Load model
    if 'logs_sample' not in args.sample_logdir:
        print("***********debug******************")
        ckpt['config']['model']['log_dir'] = "." + ckpt['config']['model']['log_dir']
    model = ScorePosNet3D(
        ckpt['config'].model,
