...
class PrepProt(object): 
    ...
    def addH(self, prot_pqr):  # call pdb2pqr
        ...
        get_cwd = os.getcwd() # get run .py 的工作目录，或者linux的工作目录
        a = "pdb2pqr" + ' --ff=AMBER ' + self.prot + ' ' + self.prot_pqr
        subprocess.Popen(["pdb2pqr", '--ff=AMBER', self.prot, self.prot_pqr],
                         stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL).communicate()

    def get_pdbqt(self, prot_pqr, prot_pdbqt):
        self.prot_pqr = prot_pqr
        prepare_receptor = os.path.join(AutoDockTools.__path__[0], 'Utilities24/prepare_receptor4.py')
        get_cwd = os.getcwd()
        a = 'python3 ' + prepare_receptor + ' -r ' + self.prot_pqr + ' -o ' + prot_pdbqt
...
class VinaDockingTask(BaseDockingTask):
    ...
    def from_generated_mol(cls, ligand_rdmol, ligand_filename, protein_root='./data/crossdocked', **kwargs):
        ...
        ) # get protein filename  and   ligand filename
        protein_path = os.path.join(protein_root, protein_fn)
        return cls(protein_path, ligand_rdmol, **kwargs) # to __init__
    ...
    def __init__(self, protein_path, ligand_rdmol, tmp_dir='./tmp', center=None,
        ...
        pos = ligand_rdmol.GetConformer(0).GetPositions()  # raw
        if center is None:
    ...
    def run(self, mode='dock', exhaustiveness=8, **kwargs):
        ...
        if not os.path.exists(protein_pdbqt):
            prot.get_pdbqt(protein_pqr, protein_pdbqt)

        # sys.exit() # 在.py文件外，执行linux的子程序
