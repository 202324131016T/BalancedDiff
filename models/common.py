...
def outer_product(*vectors):
    for index, vector in enumerate(vectors): # edge_attr:[N*E_all*k, 4] dist_feat:[N*E_all*k, 20]
        if index == 0:
            out = vector.unsqueeze(-1) # edge_attr:[N*E_all*k, 4, 1]
        else:
            out = out * vector.unsqueeze(1) # edge_attr:[N*E_all*k, 4, 1] * dist_feat:[N*E_all*k, 20] = [N*E_all*k, 4, 20]
            out = out.view(out.shape[0], -1).unsqueeze(-1)
    return out.squeeze() # [N*E_all*k, 80]
...
def compose_context(h_protein, h_ligand, pos_protein, pos_ligand, batch_protein, batch_ligand):
    ...
    ], dim=0)[sort_idx] # False:protein element, True:ligand element [N*E + N*E, ]

    batch_ctx = batch_ctx[sort_idx]
    h_ctx = torch.cat([h_protein, h_ligand], dim=0)[sort_idx]        # (N*E_protein+N_ligand, 128)
    pos_ctx = torch.cat([pos_protein, pos_ligand], dim=0)[sort_idx]  # (N*E_protein+N_ligand, 3)

    return h_ctx, pos_ctx, batch_ctx, mask_ligand # 合并feature和position
